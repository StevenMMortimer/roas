...
#'
#' @usage list_request(account, username, password)
#' @concept api list
#' @include utils.R
...
#' @param search_criteria_attributes a named character vector of attributes to add to the SearchCriteria node.
#' Acceptable parameters are pageSize and pageIndex to support paginated requests. Default pageSize for request types Campaign, 
...
#' @return A \code{data.frame} listing all objects of the specified type that also met the supplied search criteria
#' @note Most objects have a unique list of searchable criteria. Please consult the API documentation for a complete coverage.
...
#' list_of_100_pages <- list_request(credentials=my_credentials, request_type='Page', search_criteria_attributes = c(pageSize="100"))
#' list_of_campaigns_by_enddate <- list_request(credentials=my_credentials, request_type='Campaign', 
#'                                              search_criteria_attributes = c(pageSize="100"), 
#'                                              search_criteria=list(newXMLNode("EndDate", attrs = c(condition = "GT"), '2014-12-31')))
...
list_request <- function(credentials, 
                         request_type=c('Advertiser', 'AdvertiserCategory', 'Affiliate',
                                        ...
                                        'CompetitiveCategory', 'ConversionProcess', 'CreativeType',
                                        'Event', 'InsertionOrder', 'Notification',
                                        'Page', 'Product', 'RichMediaTemplate', 'SalesPerson', 'Section', 'Site',
                                        'SiteGroup', 'Transaction', 'Position',
                                        'Keyword', 'Keyname', 'Publisher', 'Campaign', 'CreativeTarget', 'Creative'),
                         search_criteria_attributes = NULL, 
                         search_criteria = NULL, verbose = FALSE){
  
  adxml_node <- newXMLNode("AdXML")
  request_node <- newXMLNode("Request", attrs = c(type = request_type), parent = adxml_node)
  database_node <- newXMLNode("Database", attrs = c(action = "list"), parent = request_node)
  search_criteria_node <- newXMLNode("SearchCriteria", attrs=search_criteria_attributes, parent = database_node)
...
list_code_request <- function(credentials,
    ...
    adxml_node <- newXMLNode("AdXML")
    request_node <- newXMLNode("Request", attrs = c(type = code_type), parent = adxml_node)
    database_node <- newXMLNode("Database", attrs = c(action = "list"), parent = request_node)
    search_criteria_node <- newXMLNode("SearchCriteria", attrs=search_criteria_attributes, parent = database_node)
    for (node in search_criteria){
      addChildren(search_criteria_node, node)
...
list_result_parser <- function(result_text, request_type, verbose = FALSE){
  ...
  errorcode <- NA
  errormessage <- NA
  try(errorcode <- xmlAttrs(getNodeSet(result_body_doc, "//Exception")[[1]], 'errorCode'), silent = T)
  try(errormessage <- xmlValue(getNodeSet(result_body_doc, "//Exception")[[1]]), silent = T)
  ...
  result_df <- xmlToDataFrame(nodes = getNodeSet(result_body_doc, paste0("//List/", request_type)))
